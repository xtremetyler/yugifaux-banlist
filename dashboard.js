...
function renderStandings(rows){
  dashboardElements.standings.innerHTML=rows.length?`<div class="standing standing--header"><span>#</span><span>Player</span><span>Record</span><span>Points</span></div>${rows.map((row,index)=>`<div class="standing"><b>${index+1}</b><a href="profiles.html?player=${encodeURIComponent(row.name)}"><strong>${escapeHtml(row.name)}</strong><small>${escapeHtml(row.archetypeName||"No active archetype")}</small></a><span><strong>${row.matchWins}-${row.matchLosses}</strong><small>${row.gamesWon}-${row.gamesLost}${row.gamesDrawn?`-${row.gamesDrawn}`:""} games</small></span><strong class="standing__points">${row.balance}<small>${row.lifetimePoints} earned</small></strong></div>`).join("")}`:empty("Standings will appear after the updated bot publishes league data.");
}
